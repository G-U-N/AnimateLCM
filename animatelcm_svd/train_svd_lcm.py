...
from animatelcm_utils import SVDSolver, update_ema, append_dims
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None, # set to none for consistency learning
...
def main():
    ...
    args.validation_images = [
        ...
        "test_imgs/woman-4549327_1280.jpg",
    ...
    if accelerator.is_main_process:
        if args.output_dir is not None:
            args.output_dir = os.path.join(args.output_dir,datetime.datetime.now().strftime("-%Y-%m-%dT%H-%M-%S"))
    ...
    target_unet = UNetSpatioTemporalConditionModel.from_pretrained(
        args.pretrained_model_name_or_path if args.pretrain_unet is None else args.pretrain_unet,
        subfolder="unet",
        low_cpu_mem_usage=True,
        variant="fp16",
    ...
    target_unet.to(accelerator.device, weight_dtype)
    ...
    if args.use_ema:
        ...
        ), model_cls=UNetSpatioTemporalConditionModel, model_config=unet.config, decay=0.999)
    ...
    train_dataset = WebVid10M(
        "/mnt/afs/wangfuyun/webvid/data/chunk0/dataset/data",
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("AnimateLCM", config=vars(args))
    ...
    def _get_add_time_ids(
        ...
        passed_add_embed_dim = unet.module.config.addition_time_embed_dim * \
            len(add_time_ids) if hasattr(unet, "module") else unet.config.addition_time_embed_dim * \
            len(add_time_ids)
        expected_add_embed_dim = unet.module.add_embedding.linear_1.in_features if hasattr(unet, "module") else unet.add_embedding.linear_1.in_features
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                indices, sigmas, timesteps, weights, c_in, c_out, c_skip = svd_solver.sample_timesteps(args.per_gpu_batch_size)
                if accelerator.is_main_process:
                ...
                encoder_hidden_states = encode_image(
                    pixel_values[:, 0, :, :, :].float()).unsqueeze(1)
                ...
                with torch.no_grad():
                    ...
                    teacher_pred_cond = teacher_unet(inp_noisy_latents, timesteps, encoder_hidden_states, added_time_ids=added_time_ids).sample
                    teacher_pred_uncond = teacher_unet(inp_noisy_latents_uncond, timesteps, encoder_hidden_states_uncond, added_time_ids=added_time_ids).sample
                    ...
                    model_pred_next = target_unet(inp_noisy_latents_next, timesteps_next, encoder_hidden_states, added_time_ids=added_time_ids).sample
            ...
            if accelerator.sync_gradients:
                ...
                update_ema(target_unet.parameters(), unet.parameters(), args.target_ema_decay)
                ...
                if accelerator.is_main_process:
                    ...
                    if (
                        ...
                        with torch.autocast(
                            ...
                            for val_img_idx, val_img_path in enumerate(args.validation_images):
                                num_frames = args.num_frames
                                video_frames = pipeline(
                                    load_image(val_img_path).resize((args.width, args.height)),
                                    height=args.height,
                                    width=args.width,
                                ...
                                out_file = os.path.join(
                                    val_save_dir,
                                    f"step_{global_step}_val_img_{val_img_idx}_2_1steps.mp4",
                                )
                            ...
                            for val_img_idx, val_img_path in enumerate(args.validation_images):
                                ...
                                out_file = os.path.join(
                                    val_save_dir,
                                    f"step_{global_step}_val_img_{val_img_idx}_2_4steps.mp4",
                                )
                            ...
                            for val_img_idx, val_img_path in enumerate(args.validation_images):
                                ...
                                out_file = os.path.join(
                                    val_save_dir,
                                    f"step_{global_step}_val_img_{val_img_idx}_1_4steps.mp4",
                                )
